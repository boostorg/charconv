...
#ifndef BOOST_CHARCONV_DETAIL_FAST_FLOAT_PARSE_NUMBER_HPP
#define BOOST_CHARCONV_DETAIL_FAST_FLOAT_PARSE_NUMBER_HPP
        ...
        if (fastfloat_strncasecmp(first, str_const_nan<UC>(), 3))
        {
            ...
            if(first != last && *first == UC('('))
            {
                for(const UC* ptr = first + 1; ptr != last; ++ptr)
                {
                    ...
                    else if(!((static_cast<UC>('a') <= *ptr && *ptr <= static_cast<UC>('z')) || 
                              (static_cast<UC>('A') <= *ptr && *ptr <= static_cast<UC>('Z')) || 
                              (static_cast<UC>('0') <= *ptr && *ptr <= static_cast<UC>('9')) || 
                              *ptr == static_cast<UC>('_')))
                    {
...
template<typename T, typename UC>
BOOST_CHARCONV_CXX20_CONSTEXPR
from_chars_result_t<UC> from_chars_advanced(const UC* first, const UC* last, T &value, parse_options_t<UC> options) noexcept  
{
    // TODO(mborland): Where things depend on float and double should have a macro to allow for
    // std::float32_t and std::float64_t when available as well.
    // They are supposed to be supported by <charconv> from C++23
    ...
    if (binary_format<T>::min_exponent_fast_path() <= pns.exponent && pns.exponent <= binary_format<T>::max_exponent_fast_path() && !pns.too_many_digits)
    {
        ...
        // be last.
        if(!BOOST_CHARCONV_IS_CONSTANT_EVALUATED(answer) && detail::rounds_to_nearest())
        {
            // We have that fegetround() == FE_TONEAREST.
            // Next is Clinger's fast path.
            if (pns.mantissa <= binary_format<T>::max_mantissa_fast_path())
            {
                value = static_cast<T>(pns.mantissa);
        ...
        else 
        {
            ...
            if (pns.exponent >= 0 && pns.mantissa <=binary_format<T>::max_mantissa_fast_path(pns.exponent))
            {
                ...
                value = static_cast<T>(pns.mantissa) * binary_format<T>::exact_power_of_ten(pns.exponent);
                if (pns.negative)
                { 
                    value = -value;
...
#endif // BOOST_CHARCONV_DETAIL_FAST_FLOAT_PARSE_NUMBER_HPP
