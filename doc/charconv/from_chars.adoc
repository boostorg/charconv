...
* long doubles support with Linux on x86 (80 or 128 bit) require GCC >= 5 with `-std=gnu\++11` or GCC >= 10 with `-std=c++11`.
Clang requires >= 9.
* long doubles are supported on x86 with all MSVC, and 64-bit platforms (e.g. ARM).
